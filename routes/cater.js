...
db.exec(`
  CREATE TABLE IF NOT EXISTS cart (
    ...
    dish_id INTEGER NOT NULL,       -- 菜品ID（关联dishes表）
    create_time DATETIME DEFAULT CURRENT_TIMESTAMP,
    update_time DATETIME DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(user_id, dish_id),       -- 同一用户对同一菜品只能有一条记录
    FOREIGN KEY (dish_id) REFERENCES dishes(id) ON DELETE CASCADE
  )
`);

db.exec(`
  DROP TABLE IF EXISTS cart_submit;
`);
...
db.exec(`
  CREATE TABLE IF NOT EXISTS cart_submit_item (
    ...
    dish_name TEXT NOT NULL,         -- 菜品名称（冗余存储，避免菜品删除后丢失）
  ...
  try {
    ...
    if (!user_id || !dish_id) {
      return res.status(400).json({
        code: 400,
        message: 'user_id 和 dish_id 不能为空'
      });
    }
    
    // // 插入或更新购物车（存在则更新数量，不存在则插入）
    // const stmt = db.prepare(`
    //   INSERT INTO cart (user_id, dish_id)
    //   VALUES (?, ?)
...
/* 清空当前用户购物车接口 */
router.delete('/cartClear', function (req, res) {
  try {
 ...
 */
router.post('/cartSubmit', function (req, res) {
  try {
    const { user_id, cart_ids = [], remark = '' } = req.body;
    ...
    const cartList = db.prepare(cartQuery).all(...queryParams);
    ...
    db.transaction(() => {
      ...
      const itemStmt = db.prepare(`
        INSERT INTO cart_submit_item (submit_id, dish_id, dish_name, quantity)
        VALUES (?, ?, ?, ?)
      `);
      cartList.forEach(item => {
        itemStmt.run(submitId, item.dish_id, item.dish_name, item.quantity);
      });
